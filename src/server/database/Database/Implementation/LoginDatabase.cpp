...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_LOGONCHALLENGE,
        ...
        "aa.gmlevel, a.totp_secret, a.s, a.v "
        "FROM account a "
    ...
    PrepareStatement(LOGIN_SEL_ACCOUNT_INFO_BY_NAME, "SELECT a.id, a.sessionkey, a.last_ip, a.locked, a.lock_country, a.expansion, a.mutetime, a.locale, a.recruiter, a.os, a.totaltime, "
    ...
    PrepareStatement(LOGIN_SEL_REALMLIST, "SELECT id, name, address, localAddress, localSubnetMask, port, icon, flag, timezone, allowedSecurityLevel, population, realmbuilds FROM realmlist WHERE flag <> 3 ORDER BY name", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_UPD_LOGON, "UPDATE account SET s = ?, v = ? WHERE id = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_INS_ACCOUNT, "INSERT INTO account(username, s, v, expansion, joindate) VALUES(?, ?, ?, ?, NOW())", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_UPD_LAST_ATTEMPT_IP, "UPDATE account SET last_ip = ? WHERE username = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_SEL_CHECK_PASSWORD, "SELECT s, v FROM account WHERE id = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_CHECK_PASSWORD_BY_NAME, "SELECT s, v FROM account WHERE username = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_PINFO, "SELECT a.username, aa.gmlevel, a.email, a.email, a.last_ip, DATE_FORMAT(a.last_login, '%Y-%m-%d %T'), a.mutetime, a.mutereason, a.muteby, a.failed_logins, a.locked, a.OS FROM account a LEFT JOIN account_access aa ON (a.id = aa.id AND (aa.RealmID = ? OR aa.RealmID = -1)) WHERE a.id = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_SEL_LAST_ATTEMPT_IP, "SELECT last_ip FROM account WHERE id = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_INS_FACL_IP_LOGGING, "INSERT INTO logs_ip_actions (account_id,character_guid,type,ip,systemnote,unixtime,time) VALUES (?, ?, ?, (SELECT last_ip FROM account WHERE id = ?), ?, unix_timestamp(NOW()), NOW())", CONNECTION_ASYNC);
